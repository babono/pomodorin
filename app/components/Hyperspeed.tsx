...
// Preset One - using turbulent distortion with cyberpunk colors
const presetOne: HyperspeedOptions = {
  onSpeedUp: () => {},
  onSlowDown: () => {},
...
class App {
  ...
  // Method to trigger speed up programmatically (for timer)
  triggerSpeedUp() {
    this.fovTarget = this.options.fovSpeedUp;
    this.speedUpTarget = this.options.speedUp;
  }

  // Method to trigger slow down programmatically (for timer)
  triggerSlowDown() {
...
const Hyperspeed: FC<HyperspeedProps> = ({ isTimerRunning = false, effectOptions = {} }) => {
  ...
  useEffect(() => {
    if (appRef.current) {
      if (isTimerRunning) {
        appRef.current.triggerSpeedUp();
      } else {
        appRef.current.triggerSlowDown();
      }
    }
  }, [isTimerRunning]);
